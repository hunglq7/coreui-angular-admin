...
export class DanhmucquatgioComponent implements OnInit {
  private gridApi!: GridApi<QuatGio>;
  dataQuatgio: QuatGio[] = [];
  constructor(
  ...
  onGridReady(params: GridReadyEvent<QuatGio>) {
  ...
  confirmDelete() {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        const selectedRows = this.gridApi.getSelectedRows();
        this.dataService
          .post('/api/Danhmucquatgio/DeleteMultipale', selectedRows)
          .subscribe({
            next: (data) => {
              this.loadDataQuatgio();
              this.toastr.success(
